...
void ObserverStats::displayStats() {
	...
	//The calculation for number of whitespace characters to be inserted below may yield a negative number under circumstances where
	//the length of the longest player name (* 2) is less than that of the length of the string generated by the current player name + information for number countries owned ( |longestName| < | currentName (24) | )
	//Added a minimum naming space for this for longest name less than 5 characters long, ( not 4 , as owning more than 99 countries would break the display again as barTitle would be > namingSpace ... 4*2 < 4 + |(xyz)| )
	int namingSpace = (longestName < 5) ? 10 : longestName*2;
