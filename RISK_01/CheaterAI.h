...
class CheaterAI : public PlayerStrategy {
public:
	//CONSTRUCTORS
	/**CheaterAI constuctor*/
	CheaterAI();

	//DESTRUCTORS
	/**CheaterAI destructor*/
	~CheaterAI();
...
private:
	...
	bool hasAttacked = false; //stores whether player attacked this round, needed to make sure AI only attacks once per round, or else they will win all games in one round

	//overrides to normal gameplay attack, reinforce and fortify methods
	bool attack(Player*);
