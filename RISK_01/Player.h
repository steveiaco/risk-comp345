...
public:
	...
	inline void promptReinforce() { strategy->reinforce(this); };
	/**Prompt the player to make an attack. Returns true if attack was successful*/
	inline bool promptAttack() { return strategy->attack(this); };
	/**Prompt the player to fortify.*/
	inline void promptFortify() { strategy->fortify(this); };
	/**Ask the player if they want to attack.*/
	inline bool askAttack() { return strategy->askAttack(this); };
	/**Ask the player where they would like to place a troop for setup.*/
	inline Country* askSetup() { return strategy->askSetup(this); };
